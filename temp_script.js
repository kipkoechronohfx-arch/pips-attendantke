    ...
    window.fetch = async function(...args) {
      ...
      if (response.status === 401 && typeof args[0] === 'string' && args[0].includes('/api/admin/') && !args[0].includes('/login') && !isLoggingOut) {
        ...
        // Show the login/setup panel in-page without reloading
        setTimeout(() => {
          isLoggingOut = false;
          if (typeof checkCredentials === 'function') checkCredentials();
        }, 500);
      }
      return response;
    };
    ...
    function checkCredentials() {
      ...
      if (adminToken && token && chat && adminKey) {
        ...
        document.getElementById('perfChartPanel').style.display = 'block';
        ...
        document.getElementById('perfChartPanel').style.display = 'none';
    ...
    // ── Copy Admin URL ─────────────────────────────────────────
    // Builds the full current admin URL (slug + key) and copies it to clipboard.
    // Shows a tick animation for 2s then resets back.
    function copyAdminUrl() {
      const key = localStorage.getItem('pa_admin_key') || '';
      const url = window.location.origin + '/admin.html' + (key ? '?key=' + encodeURIComponent(key) : '');

      navigator.clipboard.writeText(url).then(() => {
        const btn  = document.getElementById('copyAdminUrlBtn');
        const icon = document.getElementById('copyAdminUrlIcon');
        const text = document.getElementById('copyAdminUrlText');

        // Swap to success state
        btn.classList.remove('border-amber-500/20', 'bg-amber-500/10', 'text-amber-400', 'hover:border-amber-400/60', 'hover:bg-amber-400/15');
        ...
        setTimeout(() => {
          ...
          btn.classList.add('border-amber-500/20', 'bg-amber-500/10', 'text-amber-400', 'hover:border-amber-400/60', 'hover:bg-amber-400/15');
          icon.setAttribute('data-feather', 'link');
          text.textContent = 'Copy Admin URL';
          feather.replace();
        }, 2000);
      }).catch(() => {
        // Fallback: show URL in a prompt so user can manually copy
        prompt('Copy your admin URL:', url);
      });
    }
    ...
    async function generateAIRationale() {
      const btn = document.getElementById('generateRationaleBtn');
      const originalText = btn.innerHTML;
      btn.innerHTML = '<i data-feather="loader" class="w-3 h-3 animate-spin inline"></i> Generating...';
      btn.disabled = true;

      try {
        const payload = {
          asset: document.getElementById('customPair').value || 'Unknown Asset',
          type: currentSignalType ? currentSignalType.toUpperCase() : 'TRADE',
        ...
        const res = await fetch('/api/admin/generate-rationale', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('pa_admin_token')}`
          },
        ...
        alert('Failed to connect to AI server: ' + err.message);
      } finally {
    ...
    async function sendBroadcast(e) {
      ...
      const targetVal = targetSelection ? targetSelection.value : 'general';
      
      let chat = generalChat;
      if (targetVal === 'vip') {
        chat = vipChat;
      } else if (targetVal === 'both') {
        chat = `${generalChat},${vipChat}`;
      }

      if ((targetVal === 'vip' || targetVal === 'both') && !vipChat) {
      ...
      const scheduleVal = document.getElementById('signalSchedule').value;
      const scheduledTime = scheduleVal ? new Date(scheduleVal).getTime() : null;
      ...
      try {
        // Multi-image support: use the uploadedImagesBase64 array populated by handleImageUpload
        if (uploadedImagesBase64 && uploadedImagesBase64.length > 1) {
          payload.imagesBase64 = uploadedImagesBase64;
          await doBackendBroadcast(payload, broadcastBtn, feedbackMsg);
        } else if (uploadedImagesBase64 && uploadedImagesBase64.length === 1) {
          payload.imageBase64 = uploadedImagesBase64[0];
          await doBackendBroadcast(payload, broadcastBtn, feedbackMsg);
        } else {
          await doBackendBroadcast(payload, broadcastBtn, feedbackMsg);
        }
    ...
    async function doBackendBroadcast(payload, btn, feedbackEl) {
      try {
        const sendTeaser = document.getElementById('sendTeaser') && document.getElementById('sendTeaser').checked;
        if (sendTeaser) payload.sendTeaser = true;
        ...
        if (data.ok) {
          ...
          // Optionally fire PWA push notification
          const pushChecked = document.getElementById('signalPushNotify');
          if (pushChecked && pushChecked.checked) {
            const asset = document.getElementById('assetPair') ? document.getElementById('assetPair').value : 'Signal';
            const adminKey = localStorage.getItem('pa_admin_key');
            const adminToken = localStorage.getItem('pa_admin_token');
            fetch('/api/admin/push/broadcast', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey, 'x-admin-token': adminToken },
              body: JSON.stringify({
                title: `📈 New Signal: ${asset}`,
                body: 'A new forex signal has just been posted. Open the app to view it!',
                url: '/premium.html'
              })
            }).then(r => r.json()).then(d => {
              if (d.ok) console.log('[Push] Sent to', d.sentCount, 'subscribers');
            }).catch(err => console.error('[Push] Failed:', err));
            pushChecked.checked = false;
          }
        } else {
    ...
    let uploadedImagesBase64 = []; // stores array of base64 strings for multi-image
    ...
    function handleImageUpload(e) {
      ...
      fileNameLabel.innerText = `${files.length} image${files.length > 1 ? 's' : ''} selected`;
      clearBtn.classList.remove('hidden');

      files.forEach((file, i) => {
        const reader = new FileReader();
        reader.onload = (ev) => {
          uploadedImagesBase64[i] = ev.target.result;
          // Show first image in TG preview
          if (i === 0) {
            tgImg.src = ev.target.result;
            tgImgContainer.classList.remove('hidden');
          }
          // Thumbnail
          const thumb = document.createElement('img');
          thumb.src = ev.target.result;
          thumb.className = 'h-12 w-12 rounded-lg object-cover border border-white/20';
    ...
    function clearImage() {
      ...
      document.getElementById('fileNameLabel').innerText = 'Choose Image(s)... (max 4)';
      document.getElementById('clearImageBtn').classList.add('hidden');

      const previewRow = document.getElementById('imagePreviewRow');
      if (previewRow) { previewRow.innerHTML = ''; previewRow.classList.add('hidden'); }
    ...
    const engTemplates = {
      ...
      noon: "☀️ *Good noon team!* \n\nMidday check-in! Let's see how the market is moving and find some great setups! 💰💪",
      afternoon: "🌤️ *Good afternoon team!* \n\nHope your day is going well. Let's finish strong and grab some pips! 💰💪",
      evening: "🌙 *Good evening team!* \n\nReviewing the charts as the day winds down. What a day it's been! 💰💪",
      london_session: "💂 *London Session Open!*\n\nVolume is kicking in. Let's stick to our trading plan and execute with discipline. 🎯💎\n\nWhat pairs are you watching?",
      new_york_session: "🗽 *New York Session Open!*\n\nThe NY session is here! Expect high volatility and great opportunities. Let's get these pips! 🎯💎",
      tokyo_session: "🗼 *Tokyo Session Open!*\n\nAsian session has started! Time to look for steady movements and clean setups. 🎯💎",
      all_sessions: "🌍 *Forex Sessions Update!*\n\nStay alert across Tokyo, London, and New York sessions today! High-probability setups can form at any time. Keep your charts ready! 🚀📈",
      custom: ""
    };
    ...
    function applyNewsTemplate(type) {
      ...
      if (type === 'CPI') {
        titleEl.value = '🚨 High Impact News: US CPI Data Release';
        msgEl.value = 'Expect extreme market volatility due to the upcoming US Consumer Price Index (CPI) release. Tighten stop losses and manage risk carefully.';
      } else if (type === 'FOMC') {
        titleEl.value = '🚨 High Impact News: FOMC Meeting / Interest Rates';
        msgEl.value = 'The Federal Reserve is about to announce interest rate decisions. Massive volatility expected. Avoid entering new positions right now.';
      } else if (type === 'NFP') {
        titleEl.value = '🚨 High Impact News: US Non-Farm Payrolls (NFP)';
        msgEl.value = 'NFP data release is imminent! The market will experience wild swings. Protect your capital and wait for a clear direction.';
      }
    }
    ...
    async function broadcastNews() {
      const adminKey = localStorage.getItem('pa_admin_key') || '';
      const adminToken = localStorage.getItem('pa_admin_token') || '';
      const botToken = localStorage.getItem('pa_bot_token');
      const generalChat = localStorage.getItem('pa_chat_id');
      const vipChat = localStorage.getItem('pa_vip_chat_id');
      
      const title = document.getElementById('newsTitle').value;
      const message = document.getElementById('newsMessage').value;
      const fb = document.getElementById('newsFeedback');
      
      if (!message.trim()) {
        fb.innerHTML = '<span class="text-rose-400">Error: Message is required.</span>';
        return;
      }
      
      fb.innerHTML = '<span class="text-amber-400">Broadcasting...</span>';
      
      try {
        const res = await fetch('/api/admin/broadcast-news', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey, 'x-admin-token': adminToken },
          body: JSON.stringify({ title, message, type: 'News', botToken, generalChat, vipChat })
        });
        const data = await res.json();
        if (data.ok) {
          fb.innerHTML = '<span class="text-emerald-400">✅ Broadcast sent successfully!</span>';
          showToast();
          document.getElementById('newsTitle').value = '';
          document.getElementById('newsMessage').value = '';
        } else {
          fb.innerHTML = `<span class="text-rose-400">❌ Error: ${data.error}</span>`;
        }
      } catch (err) {
        fb.innerHTML = `<span class="text-rose-400">❌ Network Error: ${err.message}</span>`;
      }
    }
    ...
    async function sendEngagement() {
      ...
      const targetVal = targetSelection ? targetSelection.value : 'general';
      let chat = generalChat;
      if (targetVal === 'vip') chat = vipChat;
      else if (targetVal === 'both') chat = `${generalChat},${vipChat}`;
      ...
      const scheduleVal = document.getElementById('engSchedule').value;
      const scheduledTime = scheduleVal ? new Date(scheduleVal).getTime() : null;
      const payload = { token, chatId: chat, text, stickerId, type: 'engagement', scheduledTime };
      const targetLabel = targetVal === 'both' ? 'General + VIP' : targetVal.toUpperCase();
      
      promptBroadcastConfirm(targetLabel, 'Engagement', text || '(Sticker/Image only)', async () => {
        btn.disabled = true;
    ...
    async function sendLiveSession() {
      ...
      const scheduleVal = document.getElementById('liveSchedule').value;
      const scheduledTime = scheduleVal ? new Date(scheduleVal).getTime() : null;
      const payload = { token, chatId: chat, text: fullMessage, type: 'live', scheduledTime };
      const targetLabel = targetVal === 'both' ? 'General + VIP' : targetVal.toUpperCase();

      promptBroadcastConfirm(targetLabel, 'Live Session', fullMessage, async () => {
        btn.disabled = true;
        btn.classList.add('opacity-75', 'cursor-not-allowed');
    ...
    async function sendPerformance() {
      ...
      const generalChat = localStorage.getItem('pa_chat_id');
      const vipChat = localStorage.getItem('pa_vip_chat_id');
      const btn = document.getElementById('perfBroadcastBtn');
      const feedbackMsg = document.getElementById('perfFeedback');

      const channelSelection = document.querySelector('input[name="perfChannel"]:checked');
      const target = channelSelection ? channelSelection.value : 'general';

      if (!token) {
        feedbackMsg.innerHTML = '<span class="text-rose-400">❌ Error: Telegram bot token missing. Re-enter credentials in Settings.</span>';
        return;
      }
      if ((target === 'general' || target === 'both') && !generalChat) {
        feedbackMsg.innerHTML = '<span class="text-rose-400">❌ Error: General chat ID missing. Check Settings.</span>';
        return;
      }
      if ((target === 'vip' || target === 'both') && !vipChat) {
        feedbackMsg.innerHTML = '<span class="text-rose-400">❌ Error: VIP chat ID missing. Add it in Settings.</span>';
        return;
      }
      ...
      btn.disabled = true;
      btn.classList.add('opacity-75', 'cursor-not-allowed');
      feedbackMsg.innerHTML = '<span class="text-sky-400 animate-pulse">📡 Broadcasting via Telegram...</span>';

      try {
        const chatsToSend = [];
        if (target === 'general' || target === 'both') chatsToSend.push(generalChat);
        if (target === 'vip' || target === 'both') chatsToSend.push(vipChat);

        const results = await Promise.allSettled(
          chatsToSend.map(chatId =>
            fetch('/api/broadcast', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ token, chatId, text: msg, type: 'performance' })
            }).then(r => r.json())
          )
        );
        ...
        if (failed.length === 0) {
          feedbackMsg.innerHTML = `<span class="text-emerald-400">✅ Weekly recap broadcast to ${succeeded} channel${succeeded > 1 ? 's' : ''}!</span>`;
          showToast();
        } else if (succeeded > 0) {
          const errMsg = failed[0].reason?.message || failed[0].value?.error || 'Unknown error';
          feedbackMsg.innerHTML = `<span class="text-amber-400">⚠️ Sent to ${succeeded}/${results.length} channel(s). Error: ${errMsg}</span>`;
        } else {
          const errMsg = failed[0].reason?.message || failed[0].value?.error || 'Telegram rejected the request';
          feedbackMsg.innerHTML = `<span class="text-rose-400">❌ Failed: ${errMsg}</span>`;
        }
      } catch (err) {
        feedbackMsg.innerHTML = `<span class="text-rose-400">❌ Network error: ${err.message}</span>`;
      } finally {
        btn.disabled = false;
        btn.classList.remove('opacity-75', 'cursor-not-allowed');
      }
    ...
    async function triggerAutoRecap() {
      const adminToken = localStorage.getItem('pa_admin_token');
      const btn = document.getElementById('autoRecapBtn');
      const feedbackEl = document.getElementById('autoRecapFeedback');

      if (!adminToken) {
        feedbackEl.innerHTML = '<span class="text-rose-400">❌ Not logged in as admin.</span>';
        return;
      }

      if (!confirm("Are you sure you want to instantly broadcast the automated recap?\n\nThis will blast the VIP channel with the past 7 days' stats.")) {
        return;
      }

      btn.disabled = true;
      btn.textContent = 'Sending...';
      feedbackEl.innerHTML = '<span class="text-sky-400 animate-pulse">📡 Triggering auto recap...</span>';

      try {
        const res = await fetch('/api/admin/trigger-weekly-recap', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-token': adminToken }
        });
        const data = await res.json();
        if (data.ok) {
          feedbackEl.innerHTML = `<span class="text-emerald-400">✅ ${data.message || 'Auto recap sent to VIP channel!'}</span>`;
          showToast();
        } else {
          feedbackEl.innerHTML = `<span class="text-rose-400">❌ ${data.error || 'Failed'}</span>`;
        }
      } catch (err) {
        feedbackEl.innerHTML = `<span class="text-rose-400">❌ Network error: ${err.message}</span>`;
      } finally {
        btn.disabled = false;
      ...
      try {
        ...
        tbody.innerHTML = data.requests.map(req => {
          ...
          return `<tr class="border-b border-white/5 hover:bg-white/3 transition">
            ...
            <td class="py-3 px-4 font-mono text-[10px] text-gray-300" title="${escapeHTML(req.txHash)}">${escapeHTML(shortTx)}</td>
            <td class="py-3 px-4 text-sky-400 text-[10px]">${escapeHTML(req.contactInfo || '—')}</td>
    ...
    async function fetchAnalytics() {
      ...
      try {
        ...
        if (data.ok) {
          // If the backend returns data.stats from a different endpoint, handle it:
          const s = data.stats || data; 

          // Core user stats
          document.getElementById('statTotalUsers').textContent = s.totalUsers ?? '--';
          const activeCount = s.activeUsers ?? s.activeVIPs ?? 0;
          document.getElementById('statActiveVIPs').textContent = activeCount ?? '--';
          document.getElementById('statExpiredUsers').textContent = s.expiredUsers ?? '--';

          const convRate = s.totalUsers > 0 ? Math.round((activeCount / s.totalUsers) * 100) : 0;
          document.getElementById('statConversionRate').textContent = `${convRate}%`;

          // Revenue population
          document.getElementById('statTotalKES').textContent = `KES ${s.totalKES?.toLocaleString() ?? '--'}`;
          document.getElementById('statTotalUSDT').textContent = `$${s.totalUSDT?.toLocaleString() ?? '--'}`;
          document.getElementById('statMrrKES').textContent = `KES ${s.mrrKES?.toLocaleString() ?? '--'}`;
          document.getElementById('statMrrUSDT').textContent = `$${s.mrrUSDT?.toLocaleString() ?? '--'}`;
    ...
    function filterUsers() {
      ...
      tbody.innerHTML = filtered.map(u => {
        ...
        const statusSpan = isVip
          ? (u.isTrial 
              ? `<span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-500/10 border border-blue-500/20 text-blue-400">Trial</span>`
        ...
        return `<tr class="border-b border-white/5 hover:bg-white/3 transition">
          <td class="py-3 px-3 text-white text-xs">${escapeHTML(u.name) || '—'}</td>
          <td class="py-3 px-3 text-gray-400 text-xs">${escapeHTML(u.email)}</td>
    ...
    function exportUsersCSV() {
      ...
      const rows = [
        ...
        ...allUsersCache.map(u => [
          ...
          u.subscriptionExpiry && u.subscriptionExpiry > now ? (u.isTrial ? 'Trial VIP' : 'VIP') : 'Free',
    ...
    async function updateLiveStream() {
      const token = localStorage.getItem('pa_admin_token');
      const videoId = document.getElementById('livestreamIdInput').value.trim();
      const fb = document.getElementById('livestreamFeedback');
      if (!videoId) {
        fb.className = 'text-xs font-semibold text-rose-400';
        fb.textContent = 'Please enter a YouTube Video ID.';
        return;
      }
      fb.className = 'text-xs font-semibold text-gray-400';
      fb.textContent = 'Updating...';
      try {
        const res = await fetch('/api/admin/livestream', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-token': token },
          body: JSON.stringify({ youtubeLiveId: videoId })
        });
        const data = await res.json();
        if (data.ok) {
          fb.className = 'text-xs font-semibold text-emerald-400';
          fb.textContent = '🔴 Stream is LIVE on VIP dashboard!';
          setTimeout(() => fb.textContent = '', 4000);
        } else {
          fb.className = 'text-xs font-semibold text-rose-400';
          fb.textContent = data.error || 'Failed to update stream.';
        }
      } catch (e) {
        fb.className = 'text-xs font-semibold text-rose-400';
        fb.textContent = 'Network error.';
    ...
    async function endLiveStream() {
      ...
      const fb = document.getElementById('livestreamFeedback');
      fb.className = 'text-xs font-semibold text-gray-400';
      fb.textContent = 'Ending stream...';
      try {
        const res = await fetch('/api/admin/livestream', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-token': token },
          body: JSON.stringify({ youtubeLiveId: '' })
        });
        const data = await res.json();
        if (data.ok) {
          document.getElementById('livestreamIdInput').value = '';
          fb.className = 'text-xs font-semibold text-gray-400';
          fb.textContent = '⚫ Stream ended. VIP dashboard now shows Offline.';
          setTimeout(() => fb.textContent = '', 4000);
        }
      } catch (e) {
        fb.className = 'text-xs font-semibold text-rose-400';
    ...
    // ==========================================
    // 🏦 PROP FIRM MANAGEMENT LOGIC
    // ==========================================

    function switchAdminTab(tab) {
      // Hide all panels
      const panels = ['dashboardPanel', 'propfirmPanel', 'leadsPanel', 'blogPanel', 'analyticsPanel', 'referralsPanel'];
      panels.forEach(id => {
      ...
      // Reset all tabs
      const tabs = ['tab-broadcast', 'tab-propfirm', 'tab-leads', 'tab-blog', 'tab-analytics', 'tab-referrals'];
      tabs.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
      ...
      let targetPanel, targetTab;
      
      if (tab === 'broadcast') {
        targetPanel = 'dashboardPanel';
        targetTab = 'tab-broadcast';
      } else if (tab === 'propfirm') {
        targetPanel = 'propfirmPanel';
        targetTab = 'tab-propfirm';
        if (typeof loadPropFirmAccounts === 'function') loadPropFirmAccounts();
      } else if (tab === 'leads') {
        targetPanel = 'leadsPanel';
        targetTab = 'tab-leads';
        if (typeof fetchLeads === 'function') fetchLeads();
      } else if (tab === 'blog') {
        targetPanel = 'blogPanel';
        targetTab = 'tab-blog';
        if (typeof fetchAdminBlogPosts === 'function') fetchAdminBlogPosts();
      } else if (tab === 'analytics') {
        targetPanel = 'analyticsPanel';
        targetTab = 'tab-analytics';
        if (typeof fetchAnalytics === 'function') fetchAnalytics();
        if (typeof fetchPerfLogs === 'function') fetchPerfLogs();
        const perfPanel = document.getElementById('perfLogsPanel');
        if (perfPanel) perfPanel.classList.remove('hidden');
      } else if (tab === 'referrals') {
        targetPanel = 'referralsPanel';
        targetTab = 'tab-referrals';
        if (typeof fetchReferrals === 'function') fetchReferrals();
      }
      
      if (targetPanel && document.getElementById(targetPanel)) {
    ...
    async function loadPropFirmAccounts() {
      const adminKey = localStorage.getItem('pa_admin_key');
      const list = document.getElementById('pfList');
      list.innerHTML = '<p class="text-xs text-gray-500 text-center py-4">Loading...</p>';
      
      try {
        const res = await fetch('/api/propfirm/admin/all', { headers: { 'x-admin-key': adminKey } });
        const data = await res.json();
        if (data.ok) {
          if (!data.accounts.length) {
            list.innerHTML = '<p class="text-xs text-gray-500 text-center py-4">No active prop firm accounts found.</p>';
            return;
          }
          list.innerHTML = data.accounts.map(acc => `
            <div class="bg-black/40 border border-white/5 rounded-xl p-3 flex justify-between items-center hover:border-white/10 transition">
              <div>
                <p class="text-[11px] font-bold text-white">${acc.userName || acc.userEmail}</p>
                <p class="text-[10px] text-gray-400">${acc.firm} • ${acc.phase} • $${acc.accountSize}</p>
                <div class="flex gap-2 mt-1 text-[10px]">
                  <span class="text-amber-400 font-bold">Target: ${acc.currentPercent}% / ${acc.targetPercent}%</span>
                  <span class="text-rose-400 font-bold">DD: ${acc.maxDrawdown}%</span>
                </div>
              </div>
              <div class="flex flex-col gap-1 items-end">
                <span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase ${acc.status === 'passed' ? 'bg-emerald-500/20 text-emerald-400' : acc.status === 'failed' ? 'bg-rose-500/20 text-rose-400' : 'bg-sky-500/20 text-sky-400'}">${acc.status}</span>
                <div class="flex gap-1 mt-1">
                  <button onclick="editPropFirm('${acc.userId}')" class="text-xs text-neon-blue hover:underline">Edit</button>
                  <button onclick="deletePropFirm('${acc.userId}')" class="text-xs text-rose-400 hover:underline">Del</button>
                </div>
              </div>
            </div>
          `).join('');
          window._pfAccounts = data.accounts; // store for editing
        } else {
          list.innerHTML = `<p class="text-xs text-rose-500 text-center py-4">${data.error}</p>`;
        }
      } catch (e) {
        list.innerHTML = `<p class="text-xs text-rose-500 text-center py-4">Error loading accounts.</p>`;
      }
    }

    function editPropFirm(userId) {
      const acc = window._pfAccounts?.find(a => a.userId === userId);
      if (!acc) return;
      document.getElementById('pfUserId').value = acc.userId;
      document.getElementById('pfFirm').value = acc.firm;
      document.getElementById('pfPhase').value = acc.phase;
      document.getElementById('pfSize').value = acc.accountSize;
      document.getElementById('pfStatus').value = acc.status;
      document.getElementById('pfTarget').value = acc.targetPercent;
      document.getElementById('pfCurrent').value = acc.currentPercent;
      document.getElementById('pfDrawdown').value = acc.maxDrawdown;
      document.getElementById('pfNotes').value = acc.notes || '';
    }

    async function savePropFirmAccount(e) {
      e.preventDefault();
      const adminKey = localStorage.getItem('pa_admin_key');
      const payload = {
        userId: document.getElementById('pfUserId').value,
        firm: document.getElementById('pfFirm').value,
        phase: document.getElementById('pfPhase').value,
        accountSize: Number(document.getElementById('pfSize').value),
        status: document.getElementById('pfStatus').value,
        targetPercent: Number(document.getElementById('pfTarget').value),
        currentPercent: Number(document.getElementById('pfCurrent').value),
        maxDrawdown: Number(document.getElementById('pfDrawdown').value),
      ...
      try {
        const res = await fetch('/api/propfirm/admin/update', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey },
          body: JSON.stringify(payload)
        });
        const data = await res.json();
        if (data.ok) {
          showToast('Prop firm account updated!');
    ...
    async function deletePropFirm(userId) {
      if (!confirm('Are you sure you want to delete this prop firm tracking account?')) return;
      const adminKey = localStorage.getItem('pa_admin_key');
      try {
        const res = await fetch(`/api/propfirm/admin/${userId}`, {
          method: 'DELETE',
    ...
    // Auto-show tabs if logged in
    document.addEventListener("DOMContentLoaded", () => {
      const k = localStorage.getItem('pa_admin_key');
      if(k) {
    ...
    async function fetchLeads() {
      ...
      try {
        const res = await fetch('/api/admin/leads', { headers: { 'x-admin-key': adminKey, 'x-admin-token': adminToken } });
        const data = await res.json();
    ...
    async function deleteLead(id) {
      ...
      const adminKey = localStorage.getItem('pa_admin_key');
      const adminToken = localStorage.getItem('pa_admin_token');
      await fetch(`/api/admin/leads/${id}`, { method: 'DELETE', headers: { 'x-admin-key': adminKey, 'x-admin-token': adminToken } });
      fetchLeads();
    }
    ...
    function exportLeadsCSV() {
      const adminKey = localStorage.getItem('pa_admin_key');
      const adminToken = localStorage.getItem('pa_admin_token');
      window.open(`/api/admin/leads/export-csv?key=${adminKey}&token=${adminToken}`, '_blank');
    }
    ...
    async function fetchAdminBlogPosts() {
      ...
      try {
        const res = await fetch('/api/admin/blog', { headers: { 'x-admin-key': adminKey, 'x-admin-token': adminToken } });
        const data = await res.json();
    ...
    async function saveBlogPost(status) {
      ...
      const adminToken = localStorage.getItem('pa_admin_token');
      const res = await fetch(url, {
        method, headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey, 'x-admin-token': adminToken },
        body: JSON.stringify(payload)
      });
      if(res.ok) {
        // Broadcast to telegram if checked and publishing
        const broadcastChecked = document.getElementById('blogBroadcastTg').checked;
        if (status === 'published' && broadcastChecked) {
          const botToken = localStorage.getItem('pa_bot_token');
          const chatId = localStorage.getItem('pa_chat_id');
          if (botToken && chatId) {
            const articleUrl = `https://www.pipsattendant.com/article/${payload.slug}`;
            const tgMsg = `📰 *New Article Published!*\n\n*${payload.title}*\n\n${payload.excerpt ? payload.excerpt + '\n\n' : ''}Read it here: ${articleUrl}`;
            try {
        ...
        document.getElementById('blogBroadcastTg').checked = false;
        
        // Reset buttons
        document.getElementById('btnBlogPublish').textContent = 'Publish';
        document.getElementById('btnBlogDraft').classList.remove('hidden');
        document.getElementById('btnBlogCancel').classList.add('hidden');
    ...
    async function editBlogPost(id) {
      ...
      if(post) {
        ...
        // Use dangerouslyPasteHTML so Quill parses and binds the content correctly for editing
        quill.clipboard.dangerouslyPasteHTML(post.content || '');
        
        // Update UI for editing
        document.getElementById('btnBlogPublish').textContent = 'Update Post';
        document.getElementById('btnBlogDraft').classList.add('hidden');
        document.getElementById('btnBlogCancel').classList.remove('hidden');
    ...
    async function deleteBlogPost(id) {
      ...
      const adminKey = localStorage.getItem('pa_admin_key');
      const adminToken = localStorage.getItem('pa_admin_token');
      await fetch(`/api/admin/blog/${id}`, { method: 'DELETE', headers: { 'x-admin-key': adminKey, 'x-admin-token': adminToken } });
      fetchAdminBlogPosts();
    }

    async function uploadBlogCover() {
      const fileInput = document.getElementById('blogCoverFile');
      const btn = document.getElementById('blogUploadBtn');
      const input = document.getElementById('blogCover');
      const file = fileInput.files[0];
      if (!file) return;
      ...
      try {
        ...
        reader.onload = async () => {
          ...
          if (data.ok && data.url) {
            ...
            if(typeof showToast !== 'undefined') showToast('Image uploaded successfully');
          } else {
    ...
    async function fetchPerfLogs() {
      try {
        ...
        // Calculate stats
        let totalPips = 0, pipsGained = 0, pipsLost = 0;
        logs.forEach(log => {
          const p = Number(log.pips) || 0;
          totalPips += p;
          if (log.result === 'Win') pipsGained += Math.abs(p);
          else if (log.result === 'Loss') pipsLost += Math.abs(p);
        });
        
        const totalMovement = pipsGained + pipsLost;
        const winRate = totalMovement > 0 ? Math.round((pipsGained / totalMovement) * 100) : 0;
        
        document.getElementById('statTotalTrades').innerText = logs.length;
        document.getElementById('statTotalPips').innerText = totalPips > 0 ? '+' + totalPips : totalPips;
        document.getElementById('statWinRate').innerText = winRate + '%';

        // Plot last 50 trades chronologically (oldest to newest left to right)
        const recentLogs = logs.slice(0, 50).reverse();
        const labels = recentLogs.map(l => new Date(l.date).toLocaleDateString(undefined, {month:'short', day:'numeric'}) + ' (' + l.asset + ')');
        const chartData = recentLogs.map(l => Number(l.pips) || 0);
        const bgColors = chartData.map(val => val >= 0 ? 'rgba(16, 185, 129, 0.8)' : 'rgba(244, 63, 94, 0.8)');
          ...
          options: {
            ...
            plugins: {
              ...
              tooltip: {
                ...
                callbacks: {
                  label: (ctx) => `${ctx.raw > 0 ? '+' : ''}${ctx.raw} pips`
                }
              }
            },
            scales: {
    ...
    async function fetchReferrals() {
      ...
      try {
        const res = await fetch('/api/admin/referrals', {
          headers: { 'x-admin-key': adminKey, 'x-admin-token': adminToken }
        });
        const data = await res.json();
        
        if (data.ok) {
          if (data.referrers.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-xs text-gray-500">No referrals yet.</td></tr>';
            return;
          }
          tbody.innerHTML = data.referrers.map((r, i) => `
            <tr class="border-b border-white/5 hover:bg-white/5 transition">
              <td class="px-4 py-3 text-white">${i + 1}</td>
              <td class="px-4 py-3 font-medium text-amber-400">${r.name}</td>
              <td class="px-4 py-3">${r.email}</td>
              <td class="px-4 py-3"><span class="bg-black/50 text-xs px-2 py-1 rounded text-gray-300 font-mono">${r.referralCode || 'N/A'}</span></td>
              <td class="px-4 py-3 text-right font-bold text-white">${r.referralCount}</td>
            </tr>
          `).join('');
        } else {
          tbody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-xs text-rose-400">Error: ${data.error}</td></tr>`;
        }
      } catch (err) {
        tbody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-xs text-rose-400">Network error.</td></tr>`;
      }
    }
    
    // ── Email Broadcast to All Leads ────────────────────────────
    async function sendEmailBroadcast() {
      const subject = document.getElementById('emailBroadcastSubject').value.trim();
      const messageHtml = document.getElementById('emailBroadcastBody').value.trim();
      const feedback = document.getElementById('emailBroadcastFeedback');
      const btn = document.getElementById('emailBroadcastBtn');

      if (!subject || !messageHtml) { feedback.innerHTML = '<span class="text-rose-400">Subject and message are required.</span>'; return; }
      if (!confirm(`Send email to ALL subscribed leads?\n\nSubject: ${subject}\n\nThis cannot be undone.`)) return;

      const adminKey = localStorage.getItem('pa_admin_key');
      const adminToken = localStorage.getItem('pa_admin_token');
      btn.disabled = true;
      feedback.innerHTML = '<span class="text-neon-blue animate-pulse">Sending...</span>';
      ...
      try {
        ...
        const data = await res.json();
        if (data.ok) {
          feedback.innerHTML = `<span class="text-emerald-400">✅ ${data.message}</span>`;
          document.getElementById('emailBroadcastSubject').value = '';
          document.getElementById('emailBroadcastBody').value = '';
        } else {
          feedback.innerHTML = `<span class="text-rose-400">Error: ${data.error || 'Failed to send'}</span>`;
        }
      } catch (err) {
        feedback.innerHTML = `<span class="text-rose-400">Network error: ${err.message}</span>`;
      } finally {
        btn.disabled = false;
      }
    }
    ...
    async function downloadFullBackup() {
      const adminKey = localStorage.getItem('pa_admin_key');
      const adminToken = localStorage.getItem('pa_admin_token');
      if (!adminToken) { alert('Please log in first.'); return; }

      const btn = event.currentTarget;
      const original = btn.innerHTML;
      btn.innerHTML = '<i data-feather="loader" class="w-3.5 h-3.5 animate-spin"></i> Exporting...';
      btn.disabled = true;

      try {
        const res = await fetch(`/api/admin/backup/export-all?token=${adminToken}`, {
          headers: { 'x-admin-key': adminKey, 'x-admin-token': adminToken }
        });
        ...
        a.href = url;
        a.download = `pips_attendant_backup_${today}.json`;
    ...
    async function fetchScheduledAlerts() {
      const list = document.getElementById('saList');
      if (!list) return;
      try {
        const token = localStorage.getItem('pa_admin_token');
        const res = await fetch('/api/admin/scheduled-alerts', { headers: { 'x-admin-token': token } });
        const data = await res.json();
        if (data.ok && data.alerts) {
          if (data.alerts.length === 0) {
            list.innerHTML = '<p class="text-gray-500 text-xs text-center py-4">No pending alerts.</p>';
            return;
          }
          list.innerHTML = data.alerts.filter(a => !a.sent).sort((a,b) => a.scheduledTime - b.scheduledTime).map(a => `
            <div class="flex justify-between items-center bg-black/40 border border-white/5 rounded-xl p-3">
              <div>
                <span class="text-[10px] font-bold text-sky-400 uppercase tracking-wider">${a.target}</span>
                <p class="text-white text-xs mt-1 mb-1 font-semibold">${new Date(a.scheduledTime).toLocaleString()}</p>
                <p class="text-gray-400 text-xs truncate max-w-[200px] sm:max-w-xs">${a.message}</p>
              </div>
              <button onclick="deleteScheduledAlert('${a._id}')" class="text-rose-400 hover:text-rose-300 p-2"><i data-feather="trash-2" class="w-4 h-4"></i></button>
            </div>
          `).join('');
          if (window.feather) feather.replace();
        }
    ...
    async function handleScheduleAlert(e) {
      e.preventDefault();
      const target = document.getElementById('saTarget').value;
      const scheduledTime = document.getElementById('saTime').value;
      const message = document.getElementById('saMessage').value;
      const feedback = document.getElementById('saFeedback');
      
      try {
        const token = localStorage.getItem('pa_admin_token');
        const res = await fetch('/api/admin/scheduled-alerts', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-token': token },
          body: JSON.stringify({ target, scheduledTime, message })
        });
        const data = await res.json();
        if (data.ok) {
          feedback.innerHTML = '<span class="text-emerald-400">Alert scheduled successfully!</span>';
          document.getElementById('scheduleAlertForm').reset();
          fetchScheduledAlerts();
          setTimeout(() => feedback.innerHTML = '', 3000);
        } else {
          feedback.innerHTML = `<span class="text-rose-400">${data.error || 'Failed to schedule'}</span>`;
        }
      } catch (err) {
        feedback.innerHTML = '<span class="text-rose-400">Server error</span>';
      }
    }
    ...
    async function deleteScheduledAlert(id) {
      ...
      try {
        const token = localStorage.getItem('pa_admin_token');
        const res = await fetch(`/api/admin/scheduled-alerts/${id}`, {
          method: 'DELETE',
          headers: { 'x-admin-token': token }
        });
        const data = await res.json();
        if (data.ok) fetchScheduledAlerts();
      } catch(err) {
        console.error(err);
      }
    }

    // ── Recent Broadcasts Logic ────────────────────────────────────
    window.recentBroadcastsData = [];

    async function loadRecentBroadcasts() {
      const listEl = document.getElementById('recentBroadcastsList');
      if (!listEl) return;
      listEl.innerHTML = '<div class="text-center py-4"><i data-feather="loader" class="w-4 h-4 animate-spin text-gray-500 mx-auto"></i></div>';
      if (window.feather) feather.replace();
      ...
      try {
        ...
        if (data.ok && data.signals && data.signals.length > 0) {
          window.recentBroadcastsData = data.signals;
          renderRecentBroadcasts(data.signals);
        } else {
          window.recentBroadcastsData = [];
          listEl.innerHTML = '<p class="text-gray-500 text-xs text-center py-4">No recent broadcasts found.</p>';
        }
      } catch (err) {
        listEl.innerHTML = '<p class="text-rose-400 text-xs text-center py-4">Failed to load broadcasts.</p>';
      }
    }

    function filterRecentBroadcasts() {
      const query = (document.getElementById('recentBroadcastsFilter').value || '').toLowerCase();
      if (!query) {
        renderRecentBroadcasts(window.recentBroadcastsData);
        return;
      }
      const filtered = window.recentBroadcastsData.filter(s => {
        const text = (s.text || '').toLowerCase();
        const type = (s.type || '').toLowerCase();
        return text.includes(query) || type.includes(query);
      });
      renderRecentBroadcasts(filtered);
    }

    function renderRecentBroadcasts(signals) {
      const listEl = document.getElementById('recentBroadcastsList');
      if (!signals || signals.length === 0) {
        listEl.innerHTML = '<p class="text-gray-500 text-xs text-center py-4">No matches found.</p>';
        return;
      }
      listEl.innerHTML = signals.map(s => `
        <div class="flex flex-col bg-black/40 border border-white/5 rounded-xl p-3 hover:bg-white/5 transition">
          <div class="flex justify-between items-center mb-1">
            <span class="text-[10px] font-bold text-sky-400 uppercase tracking-wider">${s.type || 'Signal'}</span>
            <span class="text-[9px] text-gray-400">${new Date(s.sentAt).toLocaleString()}</span>
          </div>
          <p class="text-gray-300 text-xs font-sans whitespace-pre-wrap line-clamp-2">${(s.text || '').substring(0, 150)}${s.text && s.text.length > 150 ? '...' : ''}</p>
        </div>
      `).join('');
    }

    // Auto-fetch alerts when admin page loads
    document.addEventListener('DOMContentLoaded', () => {
      fetchScheduledAlerts();
    ...
    // ── Pending Broadcasts Logic ────────────────────────────────────
    async function loadPendingBroadcasts() {
      const listEl = document.getElementById('pendingBroadcastsList');
      if (!listEl) return;
      listEl.innerHTML = '<div class="text-center py-4"><i data-feather="loader" class="w-4 h-4 animate-spin text-gray-500 mx-auto"></i></div>';
      if (window.feather) feather.replace();
      try {
        const res = await fetch('/api/admin/scheduled-broadcasts', {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('pa_admin_token')}` }
        });
        const data = await res.json();
        const broadcasts = data.broadcasts || [];
        if (broadcasts.length > 0) {
          listEl.innerHTML = broadcasts.map(b => {
            const eta = b.scheduledTime ? new Date(b.scheduledTime).toLocaleString() : 'Unknown time';
            const type = b.type ? b.type.toUpperCase() : 'BROADCAST';
            const preview = (b.text || '(Image/Sticker only)').substring(0, 120);
            return `
              <div class="flex items-start justify-between bg-black/40 border border-amber-500/10 rounded-xl p-3 hover:bg-amber-500/5 transition gap-3" id="pending-${b._id}">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1">
                    <span class="text-[10px] font-bold text-amber-400 uppercase tracking-wider">${type}</span>
                    <span class="text-[9px] text-gray-500">→ ${eta}</span>
                  </div>
                  <p class="text-gray-300 text-xs truncate">${preview}</p>
                </div>
                <button onclick="cancelPendingBroadcast('${b._id}')" title="Cancel this scheduled broadcast" class="flex-shrink-0 p-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/30 text-rose-400 transition cursor-pointer">
                  <i data-feather="trash-2" class="w-3.5 h-3.5"></i>
                </button>
              </div>`;
          }).join('');
          ...
          listEl.innerHTML = '<p class="text-gray-500 text-xs text-center py-4 flex items-center justify-center gap-1.5"><i data-feather="check-circle" class="w-3.5 h-3.5 text-emerald-500"></i> No pending scheduled broadcasts.</p>';
          if (window.feather) feather.replace();
        }
      } catch (err) {
        listEl.innerHTML = '<p class="text-rose-400 text-xs text-center py-4">Failed to load pending broadcasts.</p>';
      }
    }

    async function cancelPendingBroadcast(id) {
      ...
      try {
        ...
        if (data.ok) {
          ...
          if (el) {
            ...
            setTimeout(() => loadPendingBroadcasts(), 350);
          }
        } else {
          alert('Failed to cancel: ' + data.error);
        }
      } catch (err) {
        alert('Error cancelling broadcast: ' + err.message);
      }
    }

    // ── Custom Template Builder ─────────────────────────────────────
    const CUSTOM_TEMPLATES_KEY = 'pa_custom_templates';
    ...
    function renderCustomTemplateButtons() {
      const row = document.getElementById('customTemplatesRow');
      if (!row) return;
      ...
      row.innerHTML = templates.map((t, i) => `
        <div class="relative group">
          <button type="button" onclick="applyCustomTemplate(${i})"
            class="w-full py-3 rounded-xl border border-neon-purple/20 bg-neon-purple/5 hover:bg-neon-purple/15 hover:border-neon-purple/50 text-neon-purple text-xs font-bold transition duration-200 cursor-pointer flex flex-col items-center gap-1">
            <span class="text-xl">${t.emoji || '📝'}</span>
            <span class="truncate max-w-[80px] text-center">${t.title}</span>
          </button>
          <button onclick="deleteCustomTemplate(${i})"
            class="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-rose-500 text-white text-[9px] hidden group-hover:flex items-center justify-center cursor-pointer font-bold z-10">✕</button>
        </div>
      `).join('');
    }
    ...
    function applyCustomTemplate(index) {
      ...
      if (textEl) { textEl.value = t.message; updateEngPreview(); }
      const fb = document.getElementById('templateFeedback');
      if (fb) { fb.style.opacity = '1'; fb.textContent = `✓ "${t.title}" Applied`; setTimeout(() => fb.style.opacity = '0', 1800); }
    }
    ...
    function saveCustomTemplate() {
      ...
      if (!title || !message) { alert('Please enter both a title and message.'); return; }
      const templates = loadCustomTemplates();
      templates.push({ emoji, title, message });
      saveCustomTemplates(templates);
      renderCustomTemplateButtons();
    ...
    let dashboardPerfChartInstance = null;
    ...
    async function loadPerformanceChart() {
      try {
        ...
        for (let d = 29; d >= 0; d--) {
          ...
          buckets[label] = { wins: 0, losses: 0, pips: 0 };
        }
        ...
        signals.forEach(s => {
          ...
          const label = sentAt.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
          if (buckets[label]) {
            const outcome = (s.outcome || '').toLowerCase();
            const pips = parseFloat(s.pips) || 0;
            if (outcome.includes('tp') || outcome.includes('win')) { buckets[label].wins++; buckets[label].pips += pips; totalWins++; totalPips += pips; }
            else if (outcome.includes('sl') || outcome.includes('loss') || outcome.includes('stop')) { buckets[label].losses++; buckets[label].pips -= pips; totalLosses++; totalPips -= pips; }
          }
        });

        const labels = Object.keys(buckets);
        const winsData = labels.map(l => buckets[l].wins);
        const lossesData = labels.map(l => buckets[l].losses);

        const total = totalWins + totalLosses;
        const winRate = total > 0 ? ((totalWins / total) * 100).toFixed(1) : '--';
        document.getElementById('perfWinRate').textContent = `Win Rate: ${winRate}%`;
        document.getElementById('perfNetPips').textContent = `Net Pips: ${totalPips >= 0 ? '+' : ''}${totalPips.toFixed(1)}`;

        const ctx = document.getElementById('perfChart');
        if (!ctx) return;
        if (dashboardPerfChartInstance) dashboardPerfChartInstance.destroy();
        dashboardPerfChartInstance = new Chart(ctx, {
          type: 'bar',
          data: {
            labels,
            datasets: [
              { label: 'Wins', data: winsData, backgroundColor: 'rgba(52, 211, 153, 0.6)', borderColor: 'rgba(52, 211, 153, 1)', borderWidth: 1, borderRadius: 4 },
              { label: 'Losses', data: lossesData, backgroundColor: 'rgba(248, 113, 113, 0.4)', borderColor: 'rgba(248, 113, 113, 0.8)', borderWidth: 1, borderRadius: 4 }
            ]
          },
          options: {
            responsive: true,
            ...
            plugins: {
              ...
              tooltip: { backgroundColor: '#0f172a', titleColor: '#e2e8f0', bodyColor: '#94a3b8', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1 }
            },
            scales: {
              x: { ticks: { color: '#6b7280', font: { size: 9 }, maxRotation: 45 }, grid: { color: 'rgba(255,255,255,0.05)' } },
              y: { ticks: { color: '#6b7280', font: { size: 10 }, stepSize: 1 }, grid: { color: 'rgba(255,255,255,0.05)' }, beginAtZero: true }
            }
          }
        });
      } catch (err) {
        console.warn('[PerfChart] Could not load chart data:', err.message);
      }
    }

    // ── Quick Pip & Risk Calculator Logic ─────────────────────────

    function calculateRisk() {
      const asset = document.getElementById('calcAsset').value;
      const lots = parseFloat(document.getElementById('calcLotSize').value) || 0;
      const entry = parseFloat(document.getElementById('calcEntry').value);
      const sl = parseFloat(document.getElementById('calcSL').value);

      const pipsOut = document.getElementById('calcPipsOutput');
      const riskOut = document.getElementById('calcRiskOutput');

      if (isNaN(entry) || isNaN(sl) || entry <= 0 || sl <= 0) {
        pipsOut.textContent = '0.0 pips';
        riskOut.textContent = '$0.00';
        return;
      }

      const diff = Math.abs(entry - sl);
      let pips = 0;
      let pipValue = 0; // standard value per 1 lot
      ...
      if (asset === 'forex') {
        ...
        pips = diff; 
        pipValue = 1; // varies, generic $1 per point
      }

      pipsOut.textContent = pips.toFixed(1) + ' pips';
      const risk = pips * pipValue * lots;
      riskOut.textContent = '$' + risk.toFixed(2);
    }
    ...
    document.addEventListener('paste', (e) => {
      ...
      if (dashboardPanel && !dashboardPanel.classList.contains('hidden')) {
        const input = document.getElementById('chartImage');
        if (input) {
          const dataTransfer = new DataTransfer();
          dataTransfer.items.add(imageFile);
          input.files = dataTransfer.files;
          if (typeof handleImage === 'function') handleImage({ target: input });
        }
      } else if (engagementPanel && !engagementPanel.classList.contains('hidden')) {
        const input = document.getElementById('engImage');
        if (input) {
          const dataTransfer = new DataTransfer();
          dataTransfer.items.add(imageFile);
          input.files = dataTransfer.files;
          if (typeof handleEngImage === 'function') handleEngImage({ target: input });
        }
      }
    });
